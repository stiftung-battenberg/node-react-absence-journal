...
function Absence() {
    ...
    function createAbsence (from, to, motif, comment) {
        axios.post(`${config.API_DOMAIN}/api/user/${id}/absence`, {
            from, 
            to,
            motif, 
            comment
                            ...
                            return (
                                <tr key={absence.id} className="text-gray-700">
                                    ...
                                    <td className="px-4 py-3 border">
                                        {!absence.validated && <ValidationModal icon={<GrCheckmark />} text="Validate Absence" handleEvent={validateAbsence} id={absence.id}/> }
                                        {!absence.validated && <EditAbsence editAbsence={editAbsence} id={absence.id} comment={absence.comment} motif={absence.motif} to={moment(absence.to).format('YYYY-MM-DDTHH:mm')} from={moment(absence.from).format('YYYY-MM-DDTHH:mm')}/>}
                                        {!absence.validated && <ValidationModal icon={<FaTrash />} text="Delete Absence" handleEvent={deleteAbsence} id={absence.id}/>}
                                    </td>
                                </tr>
                            )
                        })}
